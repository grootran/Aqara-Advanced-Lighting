...
`;let Et=class extends ae{constructor(){super(...arguments),this._allFavorites=[],this._loaded=!1,this._curationDrag=null,this._loadToken=0,this._t=_e.card,this._onCurationDragStart=(e,t)=>{e.preventDefault(),e.stopPropagation(),this._curationDrag={fromIdx:t,toIdx:t},e.currentTarget.setPointerCapture(e.pointerId),window.addEventListener("pointermove",this._onCurationDragMove),window.addEventListener("pointerup",this._onCurationDragEnd)},this._onCurationDragMove=e=>{if(!this._curationDrag)return;const t=this.shadowRoot.querySelectorAll(".curation-row");for(let i=0;i<t.length;i++){const s=t[i].getBoundingClientRect();if(e.clientY>=s.top&&e.clientY<=s.bottom){this._curationDrag={...this._curationDrag,toIdx:i};break}}},this._onCurationDragEnd=()=>{window.removeEventListener("pointermove",this._onCurationDragMove),window.removeEventListener("pointerup",this._onCurationDragEnd);const e=this._curationDrag;if(this._curationDrag=null,!e||e.fromIdx===e.toIdx)return;const t=this._config?.preset_ids?[...this._config.preset_ids]:[],i=new Set(t),s=[];if(this._config?.preset_ids)for(const e of this._config.preset_ids){const t=this._allFavorites.find(t=>t.ref.id===e);t&&s.push(t)}for(const e of this._allFavorites)i.has(e.ref.id)||s.push(e);const[r]=s.splice(e.fromIdx,1);if(!r)return;s.splice(e.toIdx,0,r);const o=s.filter(e=>i.has(e.ref.id)).map(e=>e.ref.id);this._updateConfig("preset_ids",o.length>0?o:void 0)}}setConfig(e){this._config={...e}}willUpdate(e){super.willUpdate(e),!this._loaded&&e.has("hass")&&this.hass?(this._loaded=!0,this._loadFavorites()):this._loaded&&e.has("_config")&&this._loadFavorites()}disconnectedCallback(){super.disconnectedCallback(),this._loadToken++,window.removeEventListener("pointermove",this._onCurationDragMove),window.removeEventListener("pointerup",this._onCurationDragEnd),this._curationDrag=null}async _loadFavorites(){if(!this.hass)return;const e=++this._loadToken;try{const[t,i,s,r]=await Promise.all([st(this.hass),rt(this.hass,{bypassCache:!0}),ot(this.hass,{bypassCache:!0}),nt(this.hass)]);if(e!==this._loadToken)return;const o=this._config?.entities??(this._config?.entity?[this._config.entity]:[]),n=new Map;for(const e of r.entities||[])n.set(e.entity_id,e.device_type);const a=Array.from(new Set(o.map(e=>n.get(e)).filter(e=>!!e))),c=0===o.length;this._allFavorites=fe(s.favorite_presets||[],t,i,a,c?{skipCompatibility:!0}:{})}catch(e){}}render(){return this.hass&&this._config?B`
      <div class="editor">
        ...
        ></ha-selector>
        ${St({label:this._t.editor.title_label,value:this._config.title||"",onInput:e=>this._updateConfig("title",e.target.value)})}
        ${void 0===this._config.layout||"grid"===this._config.layout||"compact-grid"===this._config.layout?St({label:this._t.editor.columns_label,type:"number",min:"0",max:"6",value:String(this._config.columns||0),onInput:e=>{const t=parseInt(e.target.value,10);this._updateConfig("columns",isNaN(t)||t<=0?void 0:t)}}):""}
        <ha-selector
          .hass=${this.hass}
