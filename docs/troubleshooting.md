...
- **T1M and T1 Strip RGB transition speed** -- The T1M Ceiling Light and T1 LED Strip do not support smooth color and brightness color hardware based transitions over long periods. For dynamic scenes or CCT sequences, the integration uses software-interpolated transitions to overcome this but the transitions will not be as smooth compared to bulbs like the T2 RGB or other lights that support long transitions natively.
- **Zigbee network traffic** -- Segment sequences and dynamic scenes that target multiple lights generate frequent Zigbee messages as each step updates every target device. On busy Zigbee networks this can cause congestion, delayed commands, or missed updates. To reduce load: use fewer simultaneous target lights, increase step durations, prefer longer transition times, and ensure your Zigbee mesh has enough router devices. If you experience reliability issues, consider splitting large scenes across staggered automations or reducing the number of concurrent sequences.
