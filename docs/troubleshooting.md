...
- **T1M and T1 Strip RGB transition speed** -- The T1M Ceiling Light and T1 LED Strip do not support smooth color and brightness hardware based transitions over long periods. For dynamic scenes or CCT sequences, the integration uses software-interpolated transitions to overcome this but the transitions will not be as smooth compared to bulbs like the T2 RGB or other lights that support long transitions natively.
